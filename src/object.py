from . import Core3d, textures, loadFile
import pygame
...
class Object3D:
    def __init__(
        ...
        self.texture = textures.Texture(
            display, pygame.image.load(texture).convert_alpha()
        )
    ...
    def rotate_object(self, axis, angle):
        if axis == "x":
            self.vert = Core3d.Core3d(
                Core3d.Core3d(Core3d.Core3d(self.vert).translateFliped(self.pos)).RotX(
                    angle
                )
            ).translate(self.pos)
        if axis == "y":
            self.vert = Core3d.Core3d(
                Core3d.Core3d(Core3d.Core3d(self.vert).translateFliped(self.pos)).RotY(
                    angle
                )
            ).translate(self.pos)
        if axis == "z":
            self.vert = Core3d.Core3d(
                Core3d.Core3d(Core3d.Core3d(self.vert).translateFliped(self.pos)).RotZ(
                    angle
                )
            ).translate(self.pos)
    ...
    def rotate_camera(self, axis, angle):
        if axis == "x":
            self.vert = Core3d.Core3d(self.vert).RotX(angle)
            self.pos = Core3d.Core3d([self.pos]).RotX(angle)[0]
        if axis == "y":
            self.vert = Core3d.Core3d(self.vert).RotY(angle)
            self.pos = Core3d.Core3d([self.pos]).RotY(angle)[0]
        if axis == "z":
            self.vert = Core3d.Core3d(self.vert).RotZ(angle)
            self.pos = Core3d.Core3d([self.pos]).RotZ(angle)[0]
    ...
    def clipTirangles(self, scene):
        ...
        for y in range(len(self.tirangles)):
            ...
            normal_indices = [
                self.tirangles[y][0][2],
                self.tirangles[y][1][2],
                self.tirangles[y][2][2],
            ]
